...
def save_rekognition_data(object_name, rekognition_response, s3, s3_bucket_name):
    ...
    file = open('/tmp/' + file_name, 'a')
    for label in rekognition_response['Labels']:
        total_labels = len(label['Instances'])
        if total_labels > 0:
            file.write('Label: ' + label['Name'] + '\n')
            file.write('Confidence: ' + str(label['Confidence']) + '\n')
            file.write('total instances: ' + str(total_labels) + '\n')
            file.write('----------' + '\n')
    file.close()
